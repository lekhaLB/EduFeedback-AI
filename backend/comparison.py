...
async def feedback(year: str = Form(...),
    ...
    given_answer = make_meaningful(extracted_text)
    ...
    query = {
        ...
        "question_number": question_number,
        "subdivision_number": subdivision_number,
        # Compare max_score and score fields
        "$expr": {"$eq": ["$max_score", "$score"]}
    }

    # Fetch matching documents
    filtered_docs = collection.find(query)

    # Extract the "text" field from the documents
    perfect_answers = [doc["text"] for doc in filtered_docs]

    # QUESTION IS HARDCODED TAKE THIS FROM THE QUESTIONS COLLECTION LATER
    ...
    if len(perfect_answers) > 0:
        ...
        query_embedding = embeddings.generate_query_embedding(given_answer)

    # Fetch embeddings from database
    page_embeddings = embeddings.fetch_page_embeddings_from_db(course)

    # Find the top 5 most similar pages
    top_pages = embeddings.find_top_n_similar_pages(
        query_embedding, page_embeddings, top_n=5)
    top_page_numbers = [page_num for page_num, _ in top_pages]

    document["page_numbers"] = top_page_numbers

    result = collection.insert_one(document)

    return {"message": evaluation, "pages": top_page_numbers}
...
async def feedback_img(year: str = Form(...),
    ...
    temp_max = result['max_score']
    question_text = result['question_text']

    print(temp_max)
    if score == temp_max:
        upload_image("answers_image_perfect", image, question_number,
                     test_number, subdivision_number, year, course)
        # IN THIS CASE DO NOT TRY PROVIDING FEEDBACK
        ...
        image_imperf = upload_image("answer_diagrams_imperfect", image,
                                    question_number, test_number, subdivision_number, year, course)
        image_perf = perfect_image(
            "answers_image_perfect", question_number, test_number, subdivision_number, year, course)
        # image_imperf = perfect_image("answer_diagrams_imperfect", question_number,test_number,subdivision_number, year, course)

        # perfect_image_base64 = encode_image(image_perf)
        # imperfect_image_base64 = encode_image(image)

        prompt = (
            "The first image is a perfect answer. "
            "The second image is an imperfect answer. Do not care about spelling errors"
            "Please give brief feedback to help improve the imperfect answer to match the perfect answer."
            f"this is the question for the answers: {question_text}"
            "use this question as context"
        )
    ...
    #     "question_number": question_number,
    #     "subdivision_number": subdivision_number,
    #     "$expr": {"$eq": ["$max_score", "$score"]}  # Compare max_score and score fields
    # }
...
async def reevaluate(
    ...
    # Fetch the original answer document
    original_record = collection.find_one(query)

    if not original_record:
        # print("No matching record found")
        return {"error": "Not possible to reevaluate. Evaluate first"}

    # print("Found record:", original_record)

    # Use the previously stored answer text
    given_answer = original_record["text"]

    # Fetch updated perfect answers from the database
    perfect_answers_query = {
        "year": year,
        "course": course,
        "test_number": test_number,
        "question_number": question_number,
        "subdivision_number": subdivision_number,
        "$expr": {"$eq": ["$max_score", "$score"]}  # Find full-score answers
    }
    perfect_answers_docs = collection.find(perfect_answers_query)
    perfect_answers = [doc["text"] for doc in perfect_answers_docs]

    # Get question text from questions collection
    question_doc = collection2.find_one({
        "year": year,
        "course_code": course,
        "test_number": test_number,
        "question_number": question_number,
    ...
    # Reevaluate the answer using the latest perfect answers
    evaluation = evaluate_with_groq(
        perfect_answers, given_answer, question_text)

    # Generate embedding for similarity search
    query_embedding = embeddings.generate_query_embedding(
        perfect_answers[0] if perfect_answers else given_answer
    )

    # Fetch relevant textbook pages
    page_embeddings = embeddings.fetch_page_embeddings_from_db(course)
    top_pages = embeddings.find_top_n_similar_pages(
        query_embedding, page_embeddings, top_n=5)
    top_page_numbers = [page_num for page_num, _ in top_pages]
    ...
    # print("Suggested pages:", top_page_numbers)

    # Update MongoDB record with the new evaluation
    update_data = {
        "$unset": {"dummy_field": ""},  # Force MongoDB to register an update
        "$set": {
            "feedback": evaluation,
            "evaluated_score": 0.8,  # to change if needed
            "page_numbers": top_page_numbers,
        }
    }
    ...
    if result.matched_count == 0:
        return {"error": "Document found but not updated. Query might not match."}
